...
let lastTrackedProductView = "";

function SizeGrid({ items }) {
  return (
    <div className="pd-sizeGrid">
      {items.map((item) => (
        <div key={`${item.label}-${item.value}`} className="pd-sizeCard">
...
/**
 * Purpose:
 * Supports serious evaluation of a single product by bringing together gallery
 * images, "Aplicaciones", "Diseño", "Material", and sizing guidance like
 * "Seleccioná medida".
 */
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const trackingKey = `${location.key}:${product.id}`;

    if (trackingKey === lastTrackedProductView) {
      return;
    }

    lastTrackedProductView = trackingKey;
  ...
  const selectedImage = gallery.includes(activeImage) ? activeImage : gallery[0];
  const currentIndex = Math.max(gallery.indexOf(selectedImage), 0);
